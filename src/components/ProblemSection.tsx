...
const cardVariants = {
  hidden: { opacity: 0, y: 30 }, // o los valores que tengas
  visible: { 
    opacity: 1, 
    y: 0, 
    transition: { 
...
// Creamos un sub-componente para la lógica del hover
interface CardProps {
  icon: ReactNode;
  title: string;
...
const ProblemCard = ({ icon, title, children }: CardProps) => {
  // Lógica para el efecto 3D que sigue al mouse
  const x = useMotionValue(0);
  const y = useMotionValue(0);

  // Transforma el movimiento del mouse en rotación 3D
  const rotateX = useTransform(y, [-30, 30], [8, -8]); // Invertido para efecto natural
  const rotateY = useTransform(x, [-30, 30], [-8, 8]);
  
  // Usa 'useSpring' para suavizar la animación y que no sea robótica
  ...
  const handleMouseMove = (event: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
    ...
    // Actualiza los motion values (dividimos para reducir la sensibilidad)
    x.set(mouseX / 10);
    y.set(mouseY / 10);
  };

  const handleMouseLeave = () => {
    // Resetea la posición al salir
    x.set(0);
    y.set(0);
  };
  ...
  return (
    <motion.div
      ...
      // Aplicamos la rotación 3D desde los springs
      style={{
        transformStyle: 'preserve-3d', // Habilita el contexto 3D
        rotateX: springRotateX,
        rotateY: springRotateY,
      }}
      whileHover={{ y: -5, scale: 1.03 }} // Elevación ligera al hacer hover
      onMouseMove={handleMouseMove}
      onMouseLeave={handleMouseLeave}
    >
      {/* Fondo de Grícula (ahora un div separado para que no rote) */}
      <div 
        className="absolute inset-0 bg-tech-grid opacity-30" 
        style={{ backgroundSize: '30px 30px' }}
      />

      {/* Glow Interactivo (reacciona al hover) */}
      <motion.div 
      ...
      {/* Contenido (con translateZ para "flotar" sobre el fondo) */}
...
export default function ProblemSection() {
  return (
    // Añadimos 'perspective' al contenedor para habilitar la vista 3D
    <section className="relative z-10 w-full bg-gris-piedra py-24 md:py-32 perspective-[1200px]">
      <div className="container mx-auto max-w-5xl px-4">
        ...
        >
          Tu Ecosistema Roto Te Cuesta Más Que Dinero
        </motion.h2>
        ...
        >
          ...
          >
            Tus datos de ventas y soporte son oro puro, pero mueren aislados sin que marketing o tu web los vean.
          </ProblemCard>

          {/* --- Tarjeta 2: Web Estática --- */}
          <ProblemCard 
            icon={<IconWebEstatica />} 
            title="Tu Web es estática."
          >
            Trata a tus clientes VIP igual que a un extraño. No está conectada a tus datos, por lo que no puede personalizar la experiencia.
          </ProblemCard>

          {/* --- Tarjeta 3: Contenido No Conecta --- */}
          <ProblemCard 
            icon={<IconContenidoNoConecta />} 
            title="Tu Contenido no conecta."
          >
            Tu equipo adivina qué quieren los clientes, en lugar de saberlo analizando los datos de soporte que ya posees.
          </ProblemCard>
