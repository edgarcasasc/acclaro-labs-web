...
const ProblemCard = ({ icon, title, children, delay, mouseX: globalMouseX, mouseY: globalMouseY }: CardProps) => {
  ...
  return (
    <motion.div
      ...
      className="group relative flex flex-col rounded-3xl border border-white/5 bg-slate-900/40 p-8 backdrop-blur-xl overflow-hidden hover:border-white/10 transition-colors duration-500"
      onMouseMove={handleMouseMove}
      onMouseLeave={handleMouseLeave}
    >
      {/* Luz interactiva de fondo siguiendo el mouse */}
      <motion.div
        className="pointer-events-none absolute -inset-px opacity-0 transition duration-500 group-hover:opacity-100"
        style={{ background: background }}
      />

      {/* Gradiente superior sutil en la tarjeta */}
      <div className="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-azul-zafiro/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>

      <div className="relative z-10 flex flex-col h-full">
        <div className="flex h-14 w-14 mb-6 items-center justify-center rounded-2xl bg-gradient-to-br from-slate-800 to-slate-900 border border-white/10 shadow-[0_4px_20px_rgba(0,0,0,0.5)] group-hover:border-azul-zafiro/50 transition-colors duration-500 relative">
          <div className="absolute inset-0 rounded-2xl bg-azul-zafiro/20 blur-xl opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
          <div className="text-white relative z-10">
            {icon}
          </div>
        </div>

        <h3 className="mb-4 text-2xl font-sans font-bold text-white tracking-tight">
          {title}
        </h3>
...
export default function ProblemSection({ lang = 'es' }: ProblemSectionProps) {
  ...
  return (
    <section
      ref={sectionRef}
      onMouseMove={handleMouseMove}
      className="relative z-10 w-full bg-slate-950 py-32 overflow-hidden group/section flex flex-col items-center"
    >

      {/* Fondo Oculto Global - Spotlight interactivo que sigue al cursor en toda la sección */}
      <motion.div
        className="pointer-events-none absolute inset-0 z-0 opacity-0 transition-opacity duration-1000 group-hover/section:opacity-100"
        style={{
          background: useMotionTemplate`
            radial-gradient(
              1000px circle at ${mouseX}px ${mouseY}px,
              rgba(225, 29, 72, 0.04), /* Luz de fondo roja súper difusa */
              transparent 80%
            )
          `
        }}
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none z-0">
        <div className="absolute -top-[20%] -right-[10%] w-[50%] h-[50%] rounded-full bg-azul-zafiro/5 blur-[120px]" />
        <div className="absolute bottom-[10%] -left-[10%] w-[40%] h-[40%] rounded-full bg-rojo-lacre/5 blur-[120px]" />
      </div>
      ...
      <div className="container mx-auto max-w-7xl px-6 relative z-10">
        ...
        <div className="max-w-3xl mx-auto text-center mb-20 relative z-10">
          <motion.div
            ...
            viewport={{ once: true }}
            className="inline-block mb-4 px-4 py-1.5 rounded-full border border-white/10 bg-white/5 backdrop-blur-md text-sm font-medium text-slate-400 tracking-wide uppercase shadow-[0_0_15px_rgba(225,29,72,0.1)]"
          >
            El Desafío
          </motion.div>

          <motion.h2
            className="text-4xl md:text-5xl lg:text-6xl font-sans font-bold tracking-tight text-white leading-tight drop-shadow-lg"
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.6, delay: 0.1 }}
          >
            {t.heading.split(':').map((part, i) => (
              <React.Fragment key={i}>
                {i === 0 ? (
                  <span className="block text-slate-500">{part}:</span>
                ) : (
                  <span className="bg-clip-text text-transparent bg-gradient-to-r from-red-100 to-slate-400">{part}</span>
                )}
              </React.Fragment>
            ))}
          </motion.h2>
        ...
        {/* Las tarjetas reaccionan de manera conjunta al pasar el mouse por la sección */}
