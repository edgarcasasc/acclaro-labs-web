...
// --- Iconos SVG ---
const CrmIcon = () => (
 <svg className="h-6 w-6 text-azul-zafiro" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
...
export function MegaMenu() {
  // --- 1. Constante de Idiomas ---
  const languages = [
    { code: 'es', name: 'Español', active: true }, // Marcamos 'es' como activo (base)
    { code: 'en', name: 'English', active: false },
    { code: 'zh', name: '中文 (简体)', active: false },
  ...
  // --- 2. Lógica para encontrar el idioma activo ---
  ...
  return (
    <div className="flex items-center gap-6">
          ...
          >
            <div className="p-6">
              ...
              <div className="flex flex-col gap-2">
                ...
                {/* Item 3 */}
                <Link href="/servicios/informes-ia" className="group rounded-md p-3 hover:bg-azul-zafiro/30 transition-colors">
                  <div className="flex items-start gap-4">
                    <div className="flex-shrink-0 pt-1"><BiIcon /></div>
                    <div>
                      <h4 className="font-semibold text-blanco-pergamino">Inteligencia de Negocio (BI) e IA</h4>
                      <p className="text-sm text-blanco-pergamino/70">Active sus datos con reportes (Tableau, Power BI) para reducir costos. AGI</p>
                    </div>
                  </div>
                </Link>
              </div>
      ...
      <Link href="/blog" className="text-sm font-medium text-blanco-pergamino/80 hover:text-blanco-pergamino transition-colors">
        Blog
      </Link>
      ...
      <Popover.Root>
        ...
        <Popover.Portal>
          <Popover.Content 
            sideOffset={15}
            align="center"
            // Usamos los mismos estilos del Popover de Servicios, pero con un ancho menor
            className="z-[51] w-screen max-w-[200px] rounded-lg bg-gris-piedra/90 backdrop-blur-md 
                      ...
                      data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95"
          >
            {/* Un padding más simple para la lista de idiomas */}
            <div className="p-2">
              <ul className="flex flex-col gap-1">
                {languages.map((lang) => (
                  <li key={lang.code}>
                    <a 
                      href="#" // Sin enlace funcional por ahora
                      onClick={(e) => e.preventDefault()} // Previene el salto de página
                      // Clases condicionales para el 'stub'
                      className={`
                        block w-full p-2 text-left text-sm rounded-md transition-colors 
                        ${lang.active 
                          ? 'bg-azul-zafiro/50 font-semibold text-blanco-pergamino' 
                          : 'text-blanco-pergamino/70 opacity-60 hover:bg-azul-zafiro/30 cursor-not-allowed'
                        }
                      `}
                      aria-disabled={!lang.active} // Deshabilita los inactivos
                    >
                      {lang.name}
                    </a>
                  </li>
