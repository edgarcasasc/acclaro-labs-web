...
const CONTENT = {
  ...
  en: {
    ...
    posts: [
      {
        slug: "escalar-cobranza-ia.en", // <--- Slug EN (Coincide con nombre de archivo)
        date: "2026-01-15",
        category: "Fintech",
  ...
  fr: {
    ...
    posts: [
      {
        slug: "caso-estudio-cobranza-ia.fr", // <--- Slug FR (Coincide con nombre de archivo)
        date: "15 JAN 2026",
        category: "Fintech",
...
export default function BlogIndex({ lang = 'es' }: BlogIndexProps) {
  ...
  // --- GENERADOR DE RUTAS POR IDIOMA ---
  const getPostLink = (slug: string) => {
    if (lang === 'es') return `/blog/${slug}`;
    return `/${lang}/blog/${slug}`; // Genera /fr/blog/slug o /en/blog/slug
  };

  const getContactLink = () => {
    if (lang === 'es') return '/contacto';
    return `/${lang}/contact`; // Genera /fr/contact o /en/contact
  };
        ...
        <div className="space-y-12">
          <AnimatePresence mode='wait'>
            {filteredPosts.map((post) => (
              ...
              >
                <Link 
                  href={post.status === 'published' ? getPostLink(post.slug) : '#'} 
